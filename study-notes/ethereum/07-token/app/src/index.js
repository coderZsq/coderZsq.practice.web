...
window.buyTokens = function () {
  ...
  Voting.deployed().then(votingInstance => {
    votingInstance.buy({ value: web3.toWei(_value, 'ether'), from: web3.eth.accounts[0] }).then(() => {
      votingInstance.tokenSold().then(amount => {
        $("#tokens-sold").html(amount.toString());
      });
      web3.eth.getBalance(votingInstance.address, (err, balance) => {
...
window.voteForCandidate = function () {
  ...
  $("#vote-tokens").val("");
  Voting.deployed().then(votingInstance => {
    votingInstance.voteForCandidate(candidateName, voteTokens, { from: web3.eth.accounts[0] }).then(() => {
      votingInstance.totalVotesFor(candidateName).then(count => {
...
window.lookupVoterInfo = function () {
  ...
  Voting.deployed().then(votingInstance => {
    votingInstance.voterDetail(_address).then(res => {
...
function populateCandidates() {
  Voting.deployed().then(votingInstance => {
    votingInstance.allCandidate().then(candidateArray => {
...
function populateCandidateVotes() {
  ...
  for (let i = 0; i < candidateNames.length; i++) {
    Voting.deployed().then(votingInstance => {
      votingInstance.totalVotesFor(candidateNames[i]).then(count => {
...
function populateTokenData() {
  Voting.deployed().then(votingInstance => {
    votingInstance.totalTokens().then(amount => {
      $("#tokens-total").html(amount.toString());
    });
    votingInstance.tokenSold().then(amount => {
      $("#tokens-sold").html(amount.toString());
    });
    votingInstance.tokenPrice().then(price => {
