...
class Compiler {
	...
	run(callback) {
		...
		const run = () => {
			// beforeRun -> run -> this.compile -> done
			//									-> 遇到err  -> afterdone
...
${other}`);
	...
	compile(callback) {
		...
		// hooks流程: beforeCompile -> compile -> make -> finishMake -> afterCompile
		// make是最终的编译过程, 但是它只是一个hook的调用, 我们要去找注册在这个钩子上的回调
		// 这里我们可以去找到 EntryPluginOptions
		// EntryPluginOptions -> EntryPlugin
