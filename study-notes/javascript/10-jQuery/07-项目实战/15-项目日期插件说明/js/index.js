...
$(function () {
    ...
    * */
    ...
    $(".body").on("click",".detail",function (){
        $(".mask").fadeIn(1000);
        //显示当前任务对应的信息:标题(title) + 详情(detailText) + 时间(time)
        //获取当前标签对应的数据,然后设置UI
        var index = $(this).parent().attr("index");
        current_index =index;
        var item = itemArray[index];
        $(".titleClass").text(item.title);
        $("textarea").val(item.detailText);
    ...
    $(".update").click(function () {
        //.......
        //(1) 先获取要操作的对象模型(数据)
        var item = itemArray[current_index];

        //(2) 根据当前页面UI中的数据来设置对象模型
        item.detailText = $("textarea").val();
